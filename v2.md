...
- -4 if you made an advance move.
- -4 for using weapons without the Assault rule in assault range

### Step 2 - Roll Defence

For each successful hit against a target unit that player must make a defence roll for each point of damage the attacking weapon has. The defence roll is reduced by the attacking weapons piercing attribute.
